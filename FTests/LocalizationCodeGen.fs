...
let TestEq(ctx, row, expect: String) =
    let generated, parse = generateRow (GenOutput.Empty ctx) ctx row
...
[<Test>]
let TestMissing() =
    let generated = GenOutput.Empty ctx
    let row = {
        key = "ex.missing"
        locales = [ "en!"; "jp!" ]
    }
    ...
    Assert.AreEqual((generateRow generated ctx row |> fst).errors.Length, 1)
    TestEq(ctx, row, """public static string ex_missing => Localization.Locale switch {
	_ => "en!",
};""")
